      ...
      <Row>
        <Col span="11">
          <div class="box-container left-box">
            <div class="top-info">
              <div>
                <Checkbox
                  v-model="willCheckAll"
                  :disabled="data1.length < 1"
                  @on-change="handleWillCheckAll"
                  >待选择</Checkbox
                >
              </div>
              <div>
                <span v-show="willCheckList.length > 0"
                  >{{ willCheckList.length }}/</span
                >
            ...
            <ul class="childName">
              <CheckboxGroup
                v-model="willCheckList"
                @on-change="checkWillAllGroupChange"
              >
                <li
                  class="child-item"
                  v-for="item in willData"
                  :key="item.key"
                  @dblclick.stop="dblToRight(item.key)"
                >
                  <Checkbox :label="item.key" @click.stop.native="() => {}">{{
                    item.title
                  }}</Checkbox>
                </li>
              </CheckboxGroup>
        ...
        <Col span="2" class="col2">
          <div class="arrow-box">
            <Button
              ...
              :disabled="hasCheckList.length < 1"
              @click="toTop"
            ></Button>
            <Button
              type="primary"
            ...
            <Button
              ...
              :disabled="willCheckList.length < 1"
              @click="toRight"
            ></Button>
            <Button
              type="primary"
        ...
        <Col span="11">
          <div class="box-container right-box">
            <div class="top-info">
              <div>
                <Checkbox v-model="hasCheckAll" @on-change="handleHasCheckAll"
                  >已选择</Checkbox
                >
              </div>
              <div>
                <span v-show="hasCheckList.length > 0"
                  >{{ hasCheckList.length }}/</span
                >
            ...
            <ul class="childName">
              <CheckboxGroup
                ...
                @on-change="checkHasAllGroupChange"
              >
                <li
                  class="child-item"
                  v-for="item in hasData"
                  :key="item.key"
                  @dblclick.stop="dblToLeft(item.key)"
                >
                  <Checkbox :label="item.key" @click.stop.native="() => {}">{{
                    item.title
                  }}</Checkbox>
...
export default {
  ...
  methods: {
    toggleExpand(item) {
      console.log(item);
      if (item.expand && !item.hasData) {
        item.loading = true;
        // this.loadData(item, (data) => {
        //   console.log(11);
        //   item.hasData = true;
        //   item.loading = false;
        //   data[0].title = "children1";
        //   item.children = data;
        // });
      }
    },
    ...
    toBottom() {
      ...
      this.targetKeys = this.targetKeys
        .concat(this.hasCheckList)
        .reduceRight((cur, next) => {
