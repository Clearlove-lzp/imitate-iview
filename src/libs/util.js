...
/**
 * @description GET请求当前页面下载文件
 * @param {String} url 下载地址
 * @param {String} customerName 文件名
 */
export const downloadFileFun = async (url, customerName) => {
...
/**
 * @description Post请求当前页面下载文件
 * @param {String} url 下载地址
 * @param {String} customerName 文件名
 */
export const downloadFileFunByPost = async (url, data, customerName) => {
  await axios.post(url, data, { responseType: "blob" }).then((res) => {
    let fileReader = new FileReader();
    fileReader.onload = (e) => {
      try {
        let jsonData = JSON.parse(e.target.result); // 说明是普通对象数据，后台转换失败
